...
```
java -jar build/libs/register-confirm-email-0.0.1-SNAPSHOT.jar
```
5. create file named "Dockerfile"

create docker image with docker file
```shell
#cd d:\workspace\register-confirm-email
#docker build -t register-confirm-email:v1 .
```

Give tag to docker image and push image to docker hub
```shell
#docker tag register-confirm-email purabdk/register-confirm-email
#docker push purabdk/register-confirm-email
```

remove docker images from local docker hub
```shell
#docker rmi docker-spring purabdk/register-confirm-email
```

docker pull image from docker hub and run it
```shell
#docker run -p 8080:8080 purabdk/register-confirm-email
```
